...
dsr <- function(data, event, fu, subgroup, ..., refdata, mp, method="normal", sig=0.95, decimals ) {
  ...
  all_data_st <- data |>
    dplyr::left_join(refdata,
                     by = dplyr::join_by("age")) |>
    dplyr::group_by(!!subgroup) |>
    dplyr::mutate(n=sum(!!event),
           d=sum(!!fu),
           cr_rate=.data$n/.data$d,
           cr_var=.data$n/.data$d^2,
           wts=.data$pop/sum(.data$pop),
           st_rate=sum(.data$wts*(!!event/!!fu)),
           st_var=sum(as.numeric((.data$wts^2)*(!!event/(!!fu )^2)))) |>
    dplyr::distinct(!!subgroup, .keep_all=TRUE) |>
    dplyr::select(!!subgroup, "n", "d", "cr_rate", "cr_var", "st_rate", "st_var")
  ...
  if(method=="gamma") {
    ...
    tmp1 <- all_data_st |>
      dplyr::mutate(
        ...
        c_lower=mp*stats::qgamma((1-sig)/2, shape=.data$cr_rate^2/(.data$cr_var))/(.data$cr_rate/.data$cr_var),
        c_upper=mp*stats::qgamma(1-((1-sig)/2), shape=1+.data$cr_rate^2/(.data$cr_var))/(.data$cr_rate/.data$cr_var),
        s_rate=mp*.data$st_rate,
        s_lower=mp*stats::qgamma((1-sig)/2, shape=.data$st_rate^2/.data$st_var)/(.data$st_rate/.data$st_var),
        s_upper=mp*stats::qgamma(1-((1-sig)/2), shape=1+(.data$st_rate^2/.data$st_var))/(.data$st_rate/.data$st_var)) |>
      dplyr::select(!!subgroup, "n", "d", "c_rate", "c_lower", "c_upper", "s_rate", "s_lower", "s_upper")
    ...
    tmp1 <- all_data_st |>
      dplyr::mutate(
        c_rate=mp*.data$cr_rate,
        c_lower=mp*exp((log(.data$cr_rate)+stats::qnorm((1-sig)/2)*sqrt(.data$cr_var)/(.data$cr_rate))),
        c_upper=mp*exp((log(.data$cr_rate)-stats::qnorm((1-sig)/2)*sqrt(.data$cr_var)/(.data$cr_rate))),
        s_rate=mp*.data$st_rate,
        s_lower=mp*exp((log(.data$st_rate)+stats::qnorm((1-sig)/2)*sqrt(.data$st_var)/(.data$st_rate))),
        s_upper=mp*exp((log(.data$st_rate)-stats::qnorm((1-sig)/2)*sqrt(.data$st_var)/(.data$st_rate)))) |>
      dplyr::select(!!subgroup, "n", "d", "c_rate", "c_lower", "c_upper", "s_rate", "s_lower", "s_upper")
